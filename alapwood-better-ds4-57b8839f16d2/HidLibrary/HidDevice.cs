...
namespace HidLibrary
{
    ...
    {
        public static bool deviceSharingModeIsExclusive = false; // updated by UI
        ...
        public readonly Exception openException; // set upon open failure within the constructor
        ...
        {
            Int32 sharingMode = deviceSharingModeIsExclusive ? 0 : NativeMethods.FileShareRead | NativeMethods.FileShareWrite;
            SafeFileHandle hidHandle;

            if (readAndWrite)
            {
                hidHandle = NativeMethods.CreateFile(devicePathName, NativeMethods.GENERIC_READ | NativeMethods.GENERIC_WRITE, sharingMode, IntPtr.Zero, NativeMethods.OpenExisting, 0, 0);
            }
            else
            {
                hidHandle = NativeMethods.CreateFile(devicePathName, 0, sharingMode, IntPtr.Zero, NativeMethods.OpenExisting, 0, 0);
            }
            return hidHandle;
        }
    }
